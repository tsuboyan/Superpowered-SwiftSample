...
/**
 ...
 deviceChannels The iOS device's built-in output channels.
 HDMIChannels HDMI output channels.
 USBChannels USB output channels.
 numberOfHDMIChannelsAvailable Number of available HDMI output channels. Read only.
 numberOfUSBChannelsAvailable Number of available USB output channels. Read only.
 headphoneAvailable Something is plugged into the iOS device's headphone socket or not. Read only.
 */
...
/**
 ...
 USBChannels Example: set USBChannels[0] to 3, to receive the input of the third USB channel on the first buffer.
 numberOfUSBChannelsAvailable Number of USB input channels.
 */
...
/**
 ...
 numberOfSamples The number of samples requested.
 samplerate The current sample rate in Hz.
 hostTime A mach timestamp, indicates when this chunk of audio will be passed to the audio output.
 */
...
/**
 ...
 delegate The object fully implementing the SuperpoweredIOSAudioIODelegate protocol. Not retained.
 preferredBufferSize The initial value for preferredBufferSizeMs. 12 is good for every iOS device (512 samples).
 preferredMinimumSamplerate The preferred minimum sample rate. 44100 or 48000 are recommended for good sound quality.
 audioSessionCategory The audio session category. Audio input is enabled for the appropriate categories only!
 channels The number of channels in the audio processing callback.
 callback The audio processing callback.
 clientdata Custom data passed to the audio processing callback.
 */
...
/**
 ...
 outputMap Map the output channels here.
 inputMap Map the input channels here.
 externalAudioDeviceName The name of the attached audio device, such as the model of the sound card.
 outputsAndInputs A human readable description about the available outputs and inputs.
 */
